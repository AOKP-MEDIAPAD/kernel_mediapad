...
/* Begin:wuxinxian w00176579 add for bq27510*/
#if defined(CONFIG_BATTERY_BQ275X0)
#include <linux/i2c/bq275x0_battery.h>
#endif
/* End:wuxinxian w00176579 add for bq27510*/

/*begin: add by liyaobing l00169718 20110212 for t1320 touchscreen*/
...
#endif
#endif
/*end: add by liyaobing l00169718 20110212 for t1320 touchscreen*/
#if defined(CONFIG_TOUCHSCREEN_S7020) || defined(CONFIG_TOUCHSCREEN_S700_MODULE)
...
#endif
/* begin: added by z00168965 for compass */
#if defined(CONFIG_SENSORS_AKM8973) || defined(CONFIG_SENSORS_AKM8973_MODULE)
#include <linux/akm8973.h>
#if S7_HWID_L3H(S7, S7301, T0)
#define GPIO_COMPASS_IRQ             147
#define GPIO_COMPASS_RESET           148
#endif
#endif
/* end: added by z00168965 for compass */
...
#define GPIO_PRICAM_ID             107
#define GPIO_SECCAM_ID1            103
#define GPIO_SECCAM_ID2            104
#define GPIO_PRICAM_VCM_PWDN       94
#endif

/*WLAN GPIO For 8660*/
#define GPIO_BCM43XX_REG_ON			36
#define GPIO_BCM43XX_HOST_WAKE_WL	33
#define GPIO_BCM43XX_WL_WAKE_HOST	34
#define GPIO_BCM43XX_WL_RST_N		28
#define GPIO_SIM_DETECT (132)
...
#define DOK_INT_GPIO 51
//#define UOK_INT_GPIO 50
//
//
#define USB_CHARGING_CTRL 145 
static int  msm_set_gpio_charger_ctrl(int on);


/*begin: add by liyaobing l00169718 20110212 for t1320 touchscreen*/
#if defined(CONFIG_TOUCHSCREEN_T1320) || defined(CONFIG_TOUCHSCREEN_T1320_MODULE) \
	|| defined(CONFIG_TOUCHSCREEN_S7020) || defined(CONFIG_TOUCHSCREEN_S7020_MODULE) \
...
static int ctp_gpio_setup(int enable)
{
	...
	if (enable) {
 		status = gpio_request(GPIO_3V3_EN, "3v3_en");
		if (status) {
			pr_err("%s:Failed to request GPIO %d\n",
						__func__, GPIO_3V3_EN);
			return status;
		}
        
		status = gpio_direction_output(GPIO_3V3_EN, 1);
		if (status) {
...
static int ctp_vbus_setup(bool enable)
{
    int ret; 
    ...
    if (enable) {
        ...
        if (IS_ERR(reg_8901_l4)) {
            pr_err("%s: regulator get of 8901_l4 failed (%ld)\n",
                __func__, PTR_ERR(reg_8901_l4));
            ret = PTR_ERR(reg_8901_l4);
            return ret;
        }

        ret = regulator_set_voltage(reg_8901_l4, 2600000, 2600000);
        if (ret) {
...
/*===========================================================================
  Function:       ctp_vbus_ctrl
  Description:    To set t1320 vbus pin voltage
  Calls:         
  Called By:      ctp_init_platform_hw
  Input:          vreg_on
  Output:         none
  Return:         none
  Others:         none
...
static int ctp_vbus_ctrl(int vreg_on)
{
	...
	if (!reg_8901_l4) {
		printk(KERN_ERR "%s: regulator 8901_l4 not found (%d)\n",
			__func__, ret);
		return ret;
	}

    if (vreg_on)
       ret= regulator_enable(reg_8901_l4);
    else 
       ret = regulator_force_disable(reg_8901_l4);  

	if (ret < 0) {
		printk(KERN_ERR "%s: vreg 8901_l4 %s failed (%d)\n",
				__func__, vreg_on ? "enable" : "disable", ret);
		return ret;
    }
...
/*===========================================================================
  Function:       ctp_exit_platform_hw
  Description:    touchscreen exit process function
  Calls:         
  Called By:      t1320_probe
  Input:          none
  Output:         none
  Return:         none
  Others:         none
...
/*===========================================================================
  Function:       ctp_init_platform_hw
  Description:    touchscreen start process function
  Calls:         
  Called By:      t1320_probe
  Input:          none
  Output:         none
...
static struct t1320 t1320_pdata = {
    ...
    .config_tp_5v= config_boost_5v_gpio,
	

};

static struct i2c_board_info t1320_board_info[] = {
	{
...
#endif
#if defined(CONFIG_TOUCHSCREEN_S7020) || defined(CONFIG_TOUCHSCREEN_S7020_MODULE) 
static struct s7020 s7020_pdata = {
	.client = NULL,
	.input_dev = NULL,
	.use_irq = 0,
	.data_reg = 0xff,
...
static struct i2c_board_info s7020_board_info[] = {
	{
        I2C_BOARD_INFO("s7020", 0x70),
        .irq           =  MSM_GPIO_TO_INT(GPIO_CTP_INT),
        .platform_data = &s7020_pdata,
	}
};
#endif
...
static struct i2c_board_info mx224_board_info[] = {
	{
...
#if S7_HWID_L3H(S7, S7301, T0)
#if defined(CONFIG_PMIC8XXX_VIBRATOR)
static struct regulator *reg_8901_l1;
static int vibrator_voltage_setup(bool enable)
{
    int ret; 
    ...
    if (enable) {
        ...
        if (IS_ERR(reg_8901_l1)) {
            pr_err("%s: regulator get of 8901_l1 failed (%ld)\n",
                __func__, PTR_ERR(reg_8901_l1));
            ret = PTR_ERR(reg_8901_l1);
            return ret;
        }

        ret = regulator_set_voltage(reg_8901_l1, 3300000, 3300000);
        if (ret) {
            pr_err("%s: regulator_set_voltage() = %d\n",
                __func__, ret);
            goto reg_put;
...
int vibrator_on = 0;
/*===========================================================================
  Function:       vibrator_voltage_ctrl
  Description:    To set vibrator voltage
  Calls:         
  Called By:      vibrator_init_platform_hw
  Input:          vreg_on
  Output:         none
  Return:         none
  Others:         none
===========================================================================*/
//static int vibrator_voltage_ctrl(int vreg_on)
int vibrator_voltage_ctrl(int vreg_on)
{
    int ret = -EINVAL;

	if (!reg_8901_l1) {
		printk(KERN_ERR "%s: regulator 8901_l1 not found (%d)\n",
			__func__, ret);
		return ret;
	}
...
#if 0
    if (vreg_on)
       ret= regulator_enable(reg_8901_l1);
    else 
       ret = regulator_disable(reg_8901_l1);  

	if (ret < 0) {
		printk(KERN_ERR "%s: vreg 8901_l1 %s failed (%d)\n",
				__func__, vreg_on ? "enable" : "disable", ret);
		return ret;
    }
#endif
	if (vreg_on && !vibrator_on) {
		if (regulator_is_enabled(reg_8901_l1) == 0) {
			ret= regulator_enable(reg_8901_l1);
			if (ret < 0) {
				printk(KERN_ERR "%s: vreg 8901_l1 enable failed (%d)\n",
					__func__, ret);
				return ret;
			}
			vibrator_on = 1;
	...
	if (!vreg_on && vibrator_on) {
		if (regulator_is_enabled(reg_8901_l1) > 0) {
			ret = regulator_disable(reg_8901_l1);  
			if (ret < 0) {
				printk(KERN_ERR "%s: vreg 8901_l1 disable failed (%d)\n",
					__func__, ret);
				return ret;
			}
			vibrator_on = 0;
...
/*===========================================================================
  ...
  Others:         none
===========================================================================*/
static void vibrator_exit_platform_hw(void)
{
	int ret = 0;
	ret = vibrator_voltage_ctrl(0);
	if (ret < 0) 
	{
		printk(KERN_ERR "%s failed to disable the vibrator regulator\n",__func__);
		return;         
	}
	vibrator_voltage_setup(0); 
}
/*===========================================================================
  Function:       ctp_init_platform_hw
  Description:    touchscreen start process function
  ...
  Others:         none
===========================================================================*/
static int vibrator_init_platform_hw(void)
{
	int ret = 0;
       
   ret = vibrator_voltage_setup(1);
    if (ret < 0) 
        return ret ;         
    ret = vibrator_voltage_ctrl(1);
	return ret;
}
#endif
#endif
...
| Description:   time delay millisecond							|
...
#if defined(CONFIG_S7_FEATURE_HANDSET) || defined(CONFIG_S7_FEATURE_HANDSET_MODULE)
...
void pmic_id_detect(struct work_struct *w)
{
...
#endif
#if defined(CONFIG_BATTERY_MSM8X60) 
       /*Begin: modified by zhangjie 00176551 20110217 for vbus is not routed to pmic */
	//.pmic_vbus_notif_init	= msm_hsusb_pmic_vbus_notif_init,
	.pmic_vbus_notif_init	= NULL,
	/*End: modified by zhangjie 00176551 20110217 for vbus is not routed to pmic */
#endif
...
#endif
#if defined(CONFIG_BATTERY_MSM8X60) || defined(CONFIG_BATTERY_MSM8X60_S7PRO)	
	.set_gpio_charger = msm_set_gpio_charger_ctrl,
#endif
};
#endif
...
#if S7_HWID_L3H(S7, S7301, B)
	rc = gpio_request(GPIO_PRICAM_ID, "CAM_GPIO");
	if (rc) {
		pr_err("%s gpio_request %d failed ret=%d\n",
			__func__,  GPIO_PRICAM_ID, rc);
		return -EINVAL;
	} else {
		gpio_direction_input(GPIO_PRICAM_ID);
	}	
#endif

/* Begin: deleted by wangaicui 00161317 20110130 for config s7 Pro Camera */
#if !S7_HWID_L3H(S7, S7301, T0) 
...
#endif
#endif
/* End: deleted by wangaicui 00161317 20110130 for config s7 Pro Camera */
	return rc;
}
...
#if S7_HWID_L3H(S7, S7301, B)
	rc = gpio_request(GPIO_SECCAM_ID1, "CAM_GPIO");
	if (rc) {
		pr_err("%s gpio_request %d failed ret=%d\n",
			__func__,  GPIO_SECCAM_ID1, rc);
		return -EINVAL;
	} else {
		gpio_direction_input(GPIO_SECCAM_ID1);
	}

	rc = gpio_request(GPIO_SECCAM_ID2, "CAM_GPIO");
	if (rc) {
		pr_err("%s gpio_request %d failed ret=%d\n",
			__func__,  GPIO_SECCAM_ID2, rc);
		return -EINVAL;
	} else {
		gpio_direction_input(GPIO_SECCAM_ID2);
	}
...
static void config_camera_off_gpios_web_cam(void)
{
	config_gpio_table(MSM_CAM_OFF);
#if S7_HWID_L3H(S7, S7301, B)
	gpio_free(GPIO_SECCAM_ID1);	
	gpio_free(GPIO_SECCAM_ID2);	
#endif
...
/* Begin: added by wangaicui 00161317 20110130 for config s7 Pro Camera */
#if defined( CONFIG_WEBCAM_MT9M114) || defined( CONFIG_WEBCAM_MT9M114_MODULE)
static struct msm_camera_sensor_flash_data flash_mt9m114 = {
	.flash_type		= MSM_CAMERA_FLASH_NONE,//modified
	.flash_src		= &msm_flash_src
};
...
struct platform_device msm_camera_sensor_webcam_mt9m114 = {
...
#endif
#if defined( CONFIG_MT9P111) || defined( CONFIG_MT9P111_MODULE)
...
/* End: added by wangaicui 00161317 20110130 for config s7 Pro Camera */
#endif 
	#if defined( CONFIG_MT9P111_LITEON) || defined( CONFIG_MT9P111_LITEON_MODULE)
static struct msm_camera_sensor_flash_data flash_mt9p111_liteon = {
	.flash_type		= MSM_CAMERA_FLASH_NONE,
	.flash_src		= &msm_flash_src
};
static struct msm_camera_sensor_info msm_camera_sensor_mt9p111_liteon_data = {
	.sensor_name	= "mt9p111_liteon",
	.sensor_reset	= 106,
	.sensor_pwd		= 105,
	.vcm_pwd		= 1,
	.vcm_enable		= 0,
	...
	.resource		= msm_camera_resources,
	.num_resources	= ARRAY_SIZE(msm_camera_resources),
	.flash_data		= &flash_mt9p111_liteon,
	.csi_if			= 1
};
struct platform_device msm_camera_sensor_mt9p111_liteon = {
	.name	= "msm_camera_mt9p111_liteon",
	.dev	= {
		.platform_data = &msm_camera_sensor_mt9p111_liteon_data,
	},
};
#endif
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
	...
	#if defined( CONFIG_MT9P111_LITEON) || defined( CONFIG_MT9P111_LITEON_MODULE)
	{
		I2C_BOARD_INFO("mt9p111_liteon", 0x7A >> 1),
	},	
	#endif
/* Begin: added by wangaicui 00161317 20110130 for config s7 Pro Camera */
#if defined( CONFIG_WEBCAM_MT9M114) || defined( CONFIG_WEBCAM_MT9M114_MODULE)
...
#define GPIO_I2C_SCL		142		
#define GPIO_I2C_SDA		141
...
static struct gpio_i2c_adpt_platform_data gpio_i2c_adpt_pdata = {
	.dely_usec = 5,				/*delay 5 usec for 100kHZ i2c clock frequency*/
...
#if S7_HWID_L3H(S7, S7301, T0)
/* connect to pmic8058 GPIO24*/
#define GPIO_LCD_PWM 0
/* connect to MSM8x60 GPIO */
#define GPIO_LCD_H_REV 126
#define GPIO_LCD_V_REV 127
#define GPIO_LCD_CABC_EN 129
#define GPIO_LCD_ID 154

#define GPIO_LCD_LED_EN   128
#define GPIO_LVDS_SHTDN  146
#if S7_HWID_L3H(S7, S7301, B)
#define GPIO_LED_VCCS_EN 62
#endif

static int lcd_S7Pro_gpio[8] = { 
          GPIO_LCD_PWM,  /*LCD_PWM*/
	    GPIO_LCD_LED_EN,  /*LCD_LED_EN*/
	    GPIO_LVDS_SHTDN, /*LVDS_SHTDN*/
#if S7_HWID_L3H(S7, S7301, B)
	    GPIO_LED_VCCS_EN,  /*LED_VCCS_EN*/
#endif
	    GPIO_LCD_H_REV,  /* LCD_H_REV */
	    GPIO_LCD_V_REV,  /*LCD_V_REV */
	    GPIO_LCD_CABC_EN,  /*LCD_CABC_EN*/
	    GPIO_LCD_ID,  /*LCD_ID*/
	    };

static struct msm_panel_common_pdata lcdc_s7Pro_panel_data = {
	.gpio_num = lcd_S7Pro_gpio, 
};

static struct platform_device lcdc_s7Pro_panel_device = {
	.name = "lcdc_s7Pro_lvds_wxga",
	.id = 0,
	.dev = {
		.platform_data = &lcdc_s7Pro_panel_data,
	}
};
#endif
...
#endif
/* begin: added by z00168965 for sensors power */
#if S7_HWID_L3L(S7, S7301, T0)   
static struct regulator *reg_8901_l3;

static int sensors_power(int vreg_on)
{
    int ret = -EINVAL;

	if (!reg_8901_l3) {
		printk(KERN_ERR "%s: regulator 8901_l3 not found (%d)\n",
			__func__, ret);
		return ret;
	}

    if (vreg_on) {
       ret= regulator_enable(reg_8901_l3);
    } else {
       ret = regulator_disable(reg_8901_l3);
    }    

	if (ret < 0) {
		printk(KERN_ERR "%s: vreg 8901_l3 %s failed (%d)\n",
				__func__, vreg_on ? "enable" : "disable", ret);
		return ret;
    }
...
static int sensors_dev_setup(bool enable)
{
    int rc;  
    if (enable) {
        reg_8901_l3 = regulator_get(NULL, "8901_l3");
        if (IS_ERR(reg_8901_l3)) {
            pr_err("%s: regulator get of 8901_l3 failed (%ld)\n",
                __func__, PTR_ERR(reg_8901_l3));
...
static void sensors_init(void)
{
	...
	rc = sensors_dev_setup(1);
    if (rc) {
		printk(KERN_ERR "%s: sensor_dev_setup failed!\n", __func__);
    }
    
    rc = sensors_power(1);
    if (rc) {
		printk(KERN_ERR "%s: sensor_power failed!\n", __func__);
    }
}
#endif
/* end: added by z00168965 for sensors power */
...
#define GSENSOR_IRQ_GPIO 63

static struct i2c_board_info lis35de_board_info[] = {
	{
		I2C_BOARD_INFO("lis35de", 0x1c),
		.irq = MSM_GPIO_TO_INT(GSENSOR_IRQ_GPIO),
	}
};

#endif
/* end: added by z00168965 for gsensor */

/* begin: added by z00168965 for compass */
#if defined(CONFIG_SENSORS_AKM8973) || defined(CONFIG_SENSORS_AKM8973_MODULE)

static struct akm8973_platform_data compass_pdata = {
  .irq = GPIO_COMPASS_IRQ,
  .reset = GPIO_COMPASS_RESET,
};

static struct i2c_board_info akm8973_board_info[] = {
	{
		I2C_BOARD_INFO("akm8973", 0x1e),
...
/* begin: added by z00168965 for gyro */
#if defined(CONFIG_GYRO_L3G4200D) || defined(CONFIG_GYRO_L3G4200D_MODULE)

#define GYRO_IRQ_GPIO 149
...
static struct i2c_board_info l3g4200d_board_info[] = {
	{
		I2C_BOARD_INFO("l3g4200d", 0x69),
		.irq = MSM_GPIO_TO_INT(GYRO_IRQ_GPIO),
        .platform_data = &l3g4200d_platform_data,
	}
};

#endif
/* end: added by z00168965 for gyro */

#if defined(CONFIG_S7_FEATURE_SENSORS_ST_LSM303DLH) || defined(CONFIG_S7_FEATURE_SENSORS_ST_LSM303DLH_MODULE)
static struct lsm303dlh_acc_platform_data lsm303dlh_acc_data = {
...
static struct lsm303dlh_mag_platform_data lsm303dlh_mag_data = {
...
static struct i2c_board_info lsm303dlh_board_info[] = {
	{
		I2C_BOARD_INFO("st303_acc", 0x32 >> 1),
		.irq = MSM_GPIO_TO_INT(63),
        .platform_data = &lsm303dlh_acc_data,
	},
	{
		I2C_BOARD_INFO("st303_compass", 0x3c >> 1),
		.irq = MSM_GPIO_TO_INT(147),
        .platform_data = &lsm303dlh_mag_data,
	},
};
#endif
...
/* <GPIO macro definiens of BT control  > */
#define GPIO_BCM4329_BT_WAKE_HOST    31
#define GPIO_BCM4329_HOST_WAKE_BT    35
#define GPIO_BCM4329_BT_RST_N    30
#define GPIO_BCM4329_BT_REG_ON    29

/* <GPIO macro definiens of BT UART  > */
#define GPIO_BCM4329_BT_UARTDM_RFR_N    56
#define GPIO_BCM4329_BT_UARTDM_CTS_N    55
#define GPIO_BCM4329_BT_UARTDM_BT_RX    54
#define GPIO_BCM4329_BT_UARTDM_BT_TX    53
...
{   
       int ret = 0;
       ret = gpio_direction_output(USB_CHARGING_CTRL,on);
       pr_err("%s set gpio %d to value %d, ret=%d\n",__func__,  USB_CHARGING_CTRL, on, ret);
       return ret;
}

static int setup_charging_gpio_status(int flag, unsigned gpio_num, int status)
{
	int ret = 0;

	if(flag) {
		ret = gpio_request(gpio_num, "charging-control");
		if (ret) {
			pr_err("%s gpio_request %d failed ret=%d\n",
				__func__,  gpio_num, ret);
			return -EINVAL;
		} else {
			gpio_direction_output(gpio_num, status);
		}	
	} else {
		gpio_free(gpio_num);
		ret = 0;
...
static struct msm_charger_platform_data msm_charger_data = {
	...
	.max_voltage = 4200,
	.min_voltage = 3200,
	.setup_charging_gpio_status = setup_charging_gpio_status,
};
...
#endif
#ifdef CONFIG_PM8058_CHARGER_S7PRO

static int s7pro_charger_detect_setup(int flag, unsigned gpio_num )
{
	int ret = 0;

	if(flag) {
		ret = gpio_request(gpio_num, "charger-detect");	
		if (ret) {
			pr_err("%s gpio_request %d failed ret=%d\n",
				__func__,  gpio_num, ret);
			return -EINVAL;
...
static int s7pro_get_charger_status(unsigned gpio_num)
{
	return (!gpio_get_value_cansleep(gpio_num));
}

static struct pm8058_charger_platform_data s7pro_charger_data  = {
	.name = "AC",
	.gpio_num = DOK_INT_GPIO,
	.chg_detection_config = s7pro_charger_detect_setup,
	.rating = 2,
	.get_charger_status = s7pro_get_charger_status,
	
};

static struct resource s7pro_charger_resources[] = {
	{
		.name  = "cblpwr0_n",
		.start = MSM_GPIO_TO_INT(DOK_INT_GPIO),	
		.end   = MSM_GPIO_TO_INT(DOK_INT_GPIO),	
		.flags = IORESOURCE_IRQ,
	},
};


//static struct pm8058_charger_platform_data s7pro_charger_data_usb  = {
//	.name = "USB",
//	.gpio_num = UOK_INT_GPIO,
//	.chg_detection_config = s7pro_charger_detect_setup,
//	.rating = 1,
//	.get_charger_status = s7pro_get_charger_status,
//	
//};
//
//static struct resource s7pro_charger_resources_usb[] = {
//	{
//		.name  = "cblpwr1_n",
//		.start = MSM_GPIO_TO_INT(UOK_INT_GPIO),	
//		.end   = MSM_GPIO_TO_INT(UOK_INT_GPIO),	
//		.flags = IORESOURCE_IRQ,
//	},
//};

static struct platform_device charger_device= {
	.name = "wall-charger-s7pro",
	.id = 1,
	.dev = { .platform_data = &s7pro_charger_data},
	.num_resources = ARRAY_SIZE(s7pro_charger_resources),
    .resource = s7pro_charger_resources,
...
static struct regulator_consumer_supply vreg_consumers_PM8058_L2_PC[] = {
	REGULATOR_SUPPLY("8058_l2_pc",		NULL),
};
...
static struct regulator_consumer_supply vreg_consumers_PM8058_L9_PC[] = {
	REGULATOR_SUPPLY("8058_l9_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8058_L10_PC[] = {
	REGULATOR_SUPPLY("8058_l10_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8058_L11_PC[] = {
	REGULATOR_SUPPLY("8058_l11_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8058_L12_PC[] = {
	REGULATOR_SUPPLY("8058_l12_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8058_L15_PC[] = {
	REGULATOR_SUPPLY("8058_l15_pc",		NULL),
};
...
static struct regulator_consumer_supply vreg_consumers_PM8058_S3_PC[] = {
	REGULATOR_SUPPLY("8058_s3_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8058_S4_PC[] = {
	REGULATOR_SUPPLY("8058_s4_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8901_L0_PC[] = {
	REGULATOR_SUPPLY("8901_l0_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8901_L1_PC[] = {
	REGULATOR_SUPPLY("8901_l1_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8901_L4_PC[] = {
	REGULATOR_SUPPLY("8901_l4_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8901_L6_PC[] = {
	REGULATOR_SUPPLY("8901_l6_pc",		NULL),
};
static struct regulator_consumer_supply vreg_consumers_PM8901_S3_PC[] = {
	REGULATOR_SUPPLY("8901_s3_pc",		NULL),
};
...
#define RPM_VREG_INIT_SMPS_PF(_id, _always_on, _pd, _sleep_selectable, \
			_min_uV, _max_uV, _init_peak_uA, _pin_ctrl, _pin_fn, \
			_freq) \
	RPM_VREG_INIT(_id, _min_uV, _max_uV, REGULATOR_MODE_FAST | \
		      REGULATOR_MODE_NORMAL | REGULATOR_MODE_IDLE | \
		      REGULATOR_MODE_STANDBY, REGULATOR_CHANGE_VOLTAGE | \
...
#define RPM_VREG_INIT_VS_PF(_id, _always_on, _pd, _sleep_selectable, _pin_ctrl, \
                      _pin_fn) \
      RPM_VREG_INIT(_id, 0, 0, REGULATOR_MODE_NORMAL | REGULATOR_MODE_IDLE, \
                  REGULATOR_CHANGE_STATUS | REGULATOR_CHANGE_MODE, 0, 0, \
...
static struct rpm_regulator_init_data rpm_regulator_init_data[] = {
	...
	RPM_LDO(PM8058_L2, 1, 1, 0, 3000000, 3000000, LDO300HMIN),
	...
	RPM_LDO(PM8058_L8,  1, 1, 0, 1800000, 1800000, LDO300HMIN),
	RPM_LDO(PM8058_L9,  1, 1, 0, 1800000, 1800000, LDO300HMIN),
	RPM_LDO(PM8058_L10, 1, 1, 0, 1800000, 2600000, LDO300HMIN),
	RPM_LDO(PM8058_L11, 1, 1, 0, 1800000, 1800000, LDO150HMIN),
	RPM_LDO(PM8058_L12, 1, 1, 0, 1800000, 1800000, LDO150HMIN),
	...
	RPM_LDO(PM8058_L15, 1, 1, 0, 2800000, 2800000, LDO300HMIN),
	...
	RPM_LDO(PM8058_L23, 0, 1, 0, 0, 0, LDO300HMIN),
	...
	RPM_PC(PM8058_L2,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L5,   0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L6,   0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L7,   0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L8,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L9,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L10,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L11,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L12,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_L15,   1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	...
	RPM_PC(PM8058_S3,  1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8058_S4,  1, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8901_L0,   0, ENABLE,  RPM_VREG_PIN_CTRL_PM8901_A0),
	RPM_PC(PM8901_L1,  0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8901_L4,  0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8901_L6,  0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
	RPM_PC(PM8901_S3,  0, SLEEP_B, RPM_VREG_PIN_CTRL_NONE),
...
static struct resource bluesleep_resources[] = {
	{
		.name    = "gpio_host_wake",
		.start    = GPIO_BCM4329_BT_WAKE_HOST,
		.end     = GPIO_BCM4329_BT_WAKE_HOST,
		.flags    = IORESOURCE_IO,
	},
	{
		.name    = "gpio_ext_wake",
		.start    = GPIO_BCM4329_HOST_WAKE_BT,
		.end    = GPIO_BCM4329_HOST_WAKE_BT,
		.flags	= IORESOURCE_IO,
	},
	{
		.name    = "host_wake",
		.start    = MSM_GPIO_TO_INT(GPIO_BCM4329_BT_WAKE_HOST),
		.end    = MSM_GPIO_TO_INT(GPIO_BCM4329_BT_WAKE_HOST),
		.flags    = IORESOURCE_IRQ,
	},
};


/* <bluesleep device data struct> */
...
/*< gpio config for BT power off and open >*/
static struct msm_gpio bt_config_power_off[] = {
    { GPIO_CFG(GPIO_BCM4329_BT_RST_N, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
        "BT SYSRST" },
    { GPIO_CFG(GPIO_BCM4329_BT_WAKE_HOST, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
        "HOST WAKE" },
    { GPIO_CFG(GPIO_BCM4329_HOST_WAKE_BT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
        "BT WAKE" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_RFR_N, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
		"UART1DM_RFR" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_CTS_N, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
		"UART1DM_CTS" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_BT_RX, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
		"UART1DM_RX" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_BT_TX, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
		"UART1DM_TX" }
};

static struct msm_gpio bt_config_power_on[] = {
    { GPIO_CFG(GPIO_BCM4329_BT_RST_N, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
        "BT SYSRST" },
    { GPIO_CFG(GPIO_BCM4329_BT_WAKE_HOST, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
        "HOST WAKE" },
    { GPIO_CFG(GPIO_BCM4329_HOST_WAKE_BT, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
        "BT WAKE" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_RFR_N, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		"UART1DM_RFR" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_CTS_N, 1, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		"UART1DM_CTS" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_BT_RX, 1, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		"UART1DM_RX" },
	{ GPIO_CFG(GPIO_BCM4329_BT_UARTDM_BT_TX, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		"UART1DM_TX" }
};

#define BT_POWER_OFF        0
#define BT_POWER_ON         1
/* indicata bluetooth power state */
static int bt_power_st = BT_POWER_OFF;

static int bluetooth_power(int bPowerOn)
{
	int rc;

    if (BT_POWER_ON == bPowerOn) 
    { 
	    if (BT_POWER_ON == bt_power_st) 
        {
           printk(KERN_DEBUG "Bluetooth power switch BRCM on but already on");
           return 0;
        }
        bt_power_st = BT_POWER_ON;

        /* set BT_REG_ON */
        rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM4329_BT_REG_ON, 0, GPIO_CFG_OUTPUT, 
                                        GPIO_CFG_PULL_UP, GPIO_CFG_2MA), 
                              GPIO_CFG_ENABLE);
        if (rc)
        {
            printk(KERN_ERR "%s: config GPIO_BCM4329_BT_REG_ON failed\n", __func__);
        }

        gpio_set_value(GPIO_BCM4329_BT_REG_ON, 1);
        ...
        if (rc < 0) 
        {
            printk(KERN_ERR  "%s: bt power on gpio config failed: %d\n", __func__, rc);
            return rc;
        }

        /* pull up BT_RST_N */
    	gpio_set_value(GPIO_BCM4329_BT_RST_N, 1);
    } 
    else if (BT_POWER_OFF == bPowerOn)
    {
        bt_power_st = BT_POWER_OFF;
        ...
        /* set BT_RET_ON */
        rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM4329_BT_REG_ON, 0, GPIO_CFG_INPUT, 
        ...
        gpio_set_value(GPIO_BCM4329_BT_REG_ON, 0);
    }
    else
    {
        printk(KERN_ERR "%s: bPowerOn is wrong para\n", __func__);
    }
...
static void __init msm8x60_bt_power_init(void)
{
    ...
    rc = gpio_request(GPIO_BCM4329_BT_RST_N, "bcm4325_wl_rstn");

    if (rc) //rc == 0 ,success
    {
        pr_err("gpio_request failed on pin %d\n", GPIO_BCM4329_BT_RST_N);
        goto exit;
    }
    printk(KERN_INFO "AllenWang Log[@%s] 002", __func__);
    rc = bluetooth_power(BT_POWER_OFF); // power up for test.
...
/* Begin: added by wangaicui 00161317 20110130 for config s7 Pro Camera */
#if defined( CONFIG_WEBCAM_MT9M114) || defined( CONFIG_WEBCAM_MT9M114_MODULE)
	&msm_camera_sensor_webcam_mt9m114,
#endif
#ifdef CONFIG_QS_S5K4E1
...
/* End: added by wangaicui 00161317 20110130 for config s7 Pro Camera */
	#if defined( CONFIG_MT9P111_LITEON) || defined( CONFIG_MT9P111_LITEON_MODULE)
	&msm_camera_sensor_mt9p111_liteon,
#endif
#endif
#ifdef CONFIG_MSM_GEMINI
	&msm_gemini_device,
...
static struct msm_adc_channels msm_adc_channels_data[] = {
	...
	{"xo_therm", CHANNEL_ADC_XOTHERM, 0, &xoadc_fn, CHAN_PATH_TYPE_NONE,
		ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE5, tdkntcgtherm_S7},
	{"xo_therm_4K", CHANNEL_ADC_XOTHERM_4K, 0, &xoadc_fn,
		CHAN_PATH_TYPE_NONE,
		ADC_CONFIG_TYPE1, ADC_CALIB_CONFIG_TYPE6, tdkntcgtherm_S7},
...
#else		
	{"hardware_version", CHANNEL_ADC_HW_VRESION, 0, &xoadc_fn, CHAN_PATH_TYPE10,
		ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE1, scale_default},
#endif
...
#if S7_HWID_L3H(S7, S7301, C)
    {"hardware_version2", CHANNEL_ADC_HW_VERSION2, 0, &xoadc_fn, CHAN_PATH_TYPE7,
        ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE1, scale_default},
#else
...
#if S7_HWID_L3H(S7, S7301, T0)    
	{"lightsensor", CHANNEL_ADC_LIGHT, 0, &xoadc_fn, CHAN_PATH_TYPE9,
		ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE1, scale_default},
#else
...
static void pmic8058_xoadc_mpp_config(void)
{
	...
	struct pm8xxx_mpp_init_info xoadc_mpps[] = {
		...
		PM8058_MPP_INIT(XOADC_MPP_5, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH9,
							AOUT_CTRL_DISABLE),
#if S7_HWID_L3H(S7, S7301, C)
		PM8058_MPP_INIT(XOADC_MPP_8, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH6,
							AOUT_CTRL_DISABLE),
#else
...
/* begin: added by z00168965 for light sensor */
#if S7_HWID_L3H(S7, S7301, C)
		PM8058_MPP_INIT(XOADC_MPP_4, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH8,
							AOUT_CTRL_DISABLE),
#else
...
static struct msm_gpio sim_detect_cfg[] = {
	  {GPIO_CFG(GPIO_SIM_DETECT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "sim_detect_irq"},
};
...
static struct sim_detect_platform_data sim_pdata = {
        .det_gpio		= GPIO_SIM_DETECT,
        .det_irq_gpio   = MSM_GPIO_TO_INT(GPIO_SIM_DETECT),
};

static int init_sim_detect(void)
{
    struct platform_device	*pdev;

    msm_gpios_enable(sim_detect_cfg, ARRAY_SIZE(sim_detect_cfg) );

    pdev = &sim_detect_device;
    pdev->dev.platform_data = &sim_pdata;
...
#if defined(CONFIG_KEYPAD_PRO)
struct platform_device button_pro = {
	.name		= "button-pro",
	.id			= -1,
};
#endif
...
/* Begin: modified by z00176551 20111223 for build problem */
...
//#ifdef CONFIG_ISL9519_CHARGER
/* End: modified by z00176551 20111223 for build problem */
...
//static void setup_max8903_cen_gpio(void)
//{
//	int rc = 0;
//
//	rc = gpio_request(MAX8903_CEN_N, "max8903-cen");
//	if (rc) {
//		pr_err("%s: gpio_request failed for max8903 cen. \n", __func__);
//		return;
//	} 
//	
//	rc = gpio_direction_output(MAX8903_CEN_N, 1);
//	if (rc) {
//		pr_err("%s gpio_direction_output %d failed ret=%d\n",
//			__func__, MAX8903_CEN_N, rc);
//		goto out;
//	}
//	return;
//
//out:
//	gpio_free(MAX8903_CEN_N);
//	return;
//}
//
//static void setup_charging_ctrl_gpio(void)
//{
//	int rc = 0;
//
//	rc = gpio_request(USB_CHARGING_CTRL, "max8903-cen");
//	if (rc) {
//		pr_err("%s: gpio_request failed for max8903 cen. \n", __func__);
//		return;
//	} 
//	
//	rc = gpio_direction_output(USB_CHARGING_CTRL, 0);
//	if (rc) {
//		pr_err("%s gpio_direction_output %d failed ret=%d\n",
//			__func__, USB_CHARGING_CTRL, rc);
//		goto out;
//	}
//
//out:
//	gpio_free(USB_CHARGING_CTRL);
//	return;
//}
...
static struct pm8xxx_vibrator_platform_data pm8058_vib_pdata = {
	...
	.init_platform_hw = vibrator_init_platform_hw,
	.exit_platform_hw = vibrator_exit_platform_hw,	
};
...
#if defined(CONFIG_PMIC8058_OTHC_S7PRO) || defined(CONFIG_PMIC8058_OTHC_S7PRO_MODULE)
...
/*
#if defined(CONFIG_PMIC8058_OTHC_S7PRO) || defined(CONFIG_PMIC8058_OTHC_S7PRO_MODULE)
	rc = gpio_request(HEADSET_DETECT_GPIO, "headset-detect");
	if (rc) {
		pr_err("%s: gpio_request failed for HEADSET_DETECT_GPIO\n",
								__func__);
	}

	rc = gpio_request(MIC_CTRL_GPIO, "headset-ctrl");
	if (rc) {
	...
	gpio_direction_input(HEADSET_DETECT_GPIO);
...
static struct i2c_board_info BQ275X0_boardinfo[] __initdata = {
	{
...
static struct i2c_board_info BS300_boardinfo[] __initdata = {
	{
		I2C_BOARD_INFO("bs300-audio",0x60),	
	},	
};	
#endif
/* End:wuxinxian w00176579 add for bs300*/

/* begin:wuxinxian w00176579 add for TPA2028D1*/
#ifdef CONFIG_TI_TPA2028D1
static struct i2c_board_info TPA2028D1_boardinfo[] __initdata = {
	{
		I2C_BOARD_INFO("tpa2028d1-spk",0x58),	//(0xB0 >> 1)
	},	
};	
#endif
/* end:wuxinxian w00176579 add for TPA2028D1*/
...
/* begin: added by z00168965 temporary for speaker */    
#if !S7_HWID_L3H(S7, S7301, T0)
...
#endif
/* end: added by z00168965 temporary for speaker */    
unget:
...
/* Begin:wuxinxian w00176579 add for bs300*/
	...
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_SSBI3_I2C_BUS_ID + 1,
		BS300_boardinfo,
		ARRAY_SIZE(BS300_boardinfo),
	},
#endif
/* End:wuxinxian w00176579 add for bs300*/

/*Begin: wuxinxian 00176579 add for TPA2028D1 */
#ifdef CONFIG_TI_TPA2028D1
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI7_QUP_I2C_BUS_ID,
		TPA2028D1_boardinfo,
		ARRAY_SIZE(TPA2028D1_boardinfo),
	},
#endif
/*End: wuxinxian 00176579 add for TPA2028D1 */
...
/* begin: added by z00168965 for gyro */
#if defined(CONFIG_GYRO_L3G4200D) || defined(CONFIG_GYRO_L3G4200D_MODULE)
	{
	    ...
	    MSM_GSBI12_QUP_I2C_BUS_ID, //i2c for sensor
	    l3g4200d_board_info,
	    ARRAY_SIZE(l3g4200d_board_info),
	},
#endif
/* end: added by z00168965 for gyro */
...
#if defined(CONFIG_S7_FEATURE_SENSORS_ST_LSM303DLH) || defined(CONFIG_S7_FEATURE_SENSORS_ST_LSM303DLH_MODULE)
	{
	    I2C_SURF,
	    MSM_GSBI12_QUP_I2C_BUS_ID, //i2c for sensor
	    lsm303dlh_board_info,
	    ARRAY_SIZE(lsm303dlh_board_info),
	},
#endif
...
static void __init msm8x60_init_buses(void)
{
    ...
    /* GSBI6(2) */
    msm_uart_dm1_pdata.wakeup_irq = gpio_to_irq(GPIO_BCM4329_BT_UARTDM_BT_RX);
...
{
#if 0 //Initial high sleep current fix?
	int rc;
#endif
	int rc=0;
	if (!vreg->enabled) {
...
static int msm_sdcc_setup_vreg(int dev_id, unsigned char enable)
{
	...
	if (curr_vdd_reg) {
		if (enable) {
			//if ("8058_l14" == curr_vdd_reg->reg_name)  
                        if(0 == strncmp(curr_vdd_reg->reg_name,"8058_l14",8))
				mdelay(1); 
...
static void *wlan_status_cb_devid = NULL; /*WLAN device id*/
static int msm8x60_wlan_cd = 0;		 	/*WLAN status*/
static int bcm43xx_init_flag = 0; 		/*WLAN gpio init flag*/

/*+==============================================================+
| Function:      msm8x60_register_status_notify		     		| 
   
| Description:   register WLAN status notify					|

| Input:         dev driver info			                      | 

| Return:      0:success			   				 	   		|
+==============================================================+*/
static int msm8x60_register_status_notify(void (*callback)(int card_present, void *dev_id),
					 					  void *dev_id)
{
	if (wlan_status_cb)
	{
		printk(KERN_ERR "%s(), wlan_status_cb is not NULL pointer, needed try again\n", __func__);
		return -EAGAIN;
	}
...
/*+==============================================================+
| Function:      msm8x60_wlan_status			           		| 
   
| Description:   get the state of the wlan card					|

| Input:         dev driver info			                      | 

| Return:      state of wlan card						   		|
+==============================================================+*/
static unsigned int msm8x60_wlan_status(struct device *dev)
{
	return msm8x60_wlan_cd;
}

/*+==============================================================+
| Function:      msm8x60_wlan_set_carddetect					| 
   
| Description:   WLAN sdio card detect enable					|

| Input:         val=1:enable; 	val=1:disable;			        | 

| Return:       0:success								   		|
+==============================================================+*/
int msm8x60_wlan_set_carddetect(int val)
{
	msm8x60_wlan_cd = val;
	if (wlan_status_cb)
...
EXPORT_SYMBOL(msm8x60_wlan_set_carddetect);

/*+==============================================================+
| Function:      brcm_wlan_oob		    						| 
   
| Description:   wlan wake oob irq initalize					|

| Input:         none									        | 

| Return:       oob_irq:success; -1:fail						   		|
+==============================================================+*/
int brcm_wlan_oob(void)
{
	int oob_irq = -1;
	gpio_direction_input(GPIO_BCM43XX_WL_WAKE_HOST);
	oob_irq = gpio_to_irq(GPIO_BCM43XX_WL_WAKE_HOST);
	return oob_irq;
}
EXPORT_SYMBOL(brcm_wlan_oob);


/*+==============================================================+
| Function:      wlan_bcm43xx_pm_init							| 
   
| Description:   wlan customer pm init							|

| Input:         none									        | 

| Return:       0:success; -1:fail						   		|
+==============================================================+*/
static int wlan_bcm43xx_pm_init(void)
{
	int rc = 0;

	if(bcm43xx_init_flag)
	{
		printk(KERN_ERR "%s(), bcm43xx PM init already\n", __func__);
		return -1;
	}
	
	if ((rc = gpio_request(GPIO_BCM43XX_REG_ON, "bcm43xx_wl_reg_on"))) {
	    pr_err("gpio_request failed on pin %d, rc=%d\n", GPIO_BCM43XX_REG_ON, rc);
	    goto err_bcm43xx_pm_init1;
	}
	
	if ((rc = gpio_request(GPIO_BCM43XX_HOST_WAKE_WL, "bcm43xx_host_wake_wl"))) {
	    pr_err("gpio_request failed on pin %d, rc=%d\n", GPIO_BCM43XX_HOST_WAKE_WL, rc);
	    goto err_bcm43xx_pm_init2;
	}

	if ((rc = gpio_request(GPIO_BCM43XX_WL_WAKE_HOST, "bcm43xx_wl_wake_host"))) 
	{
		pr_err("gpio_request failed on pin %d\n", GPIO_BCM43XX_WL_WAKE_HOST);
		goto err_bcm43xx_pm_init3;
	}
    
	if ((rc = gpio_request(GPIO_BCM43XX_WL_RST_N, "bcm43xx_wl_rstn"))) {
	    pr_err("gpio_request failed on pin %d\n", GPIO_BCM43XX_WL_RST_N);
	    goto err_bcm43xx_pm_init4;
	}

    bcm43xx_init_flag = 1;
	return 0;
	
err_bcm43xx_pm_init4:
	gpio_free(GPIO_BCM43XX_WL_WAKE_HOST);
err_bcm43xx_pm_init3:
	gpio_free(GPIO_BCM43XX_HOST_WAKE_WL);
err_bcm43xx_pm_init2:
	gpio_free(GPIO_BCM43XX_REG_ON);
err_bcm43xx_pm_init1:
    bcm43xx_init_flag = 0;
	printk(KERN_ERR "%s(),rc=%d, fail\n", __func__, rc);
    return -1;
}

#if 0
/*+==============================================================+
| Function:      Wlan_VREG_L8A_cfg								| 
   
| Description:   config wlan pull up power VREG_L8A				|

| Input:         1:enable VREG_L8A; 0 disable VREG_L8A	        | 

| Return:       0:success; -1:fail						   		|
+==============================================================+*/
static int Wlan_VREG_cfg(int enable)
{
	int rc = 0;
	#if 0
	const char *msm_wlan_regulator = "8058_l8";
	static struct regulator *vreg_8058_l8;
	
	vreg_8058_l8 = regulator_get(NULL, msm_wlan_regulator);
	if (IS_ERR(vreg_8058_l8)) {
		rc = PTR_ERR(vreg_8058_l8);
		pr_err("%s: regulator_get %s = %d\n", __func__,	msm_wlan_regulator, rc);
	}
	
#if S7_HWID_L3H(S7, S7301, C) 	/*Set 8058_L8A as 1.8V for VC*/
	rc = regulator_set_voltage(vreg_8058_l8, 1800000, 1800000);
#elif S7_HWID_L3H(S7, S7301, B) 	/*Set 8058_L8A as 2.6V for VB*/
	rc = regulator_set_voltage(vreg_8058_l8, 2600000, 2600000);
#else
	rc = regulator_set_voltage(vreg_8058_l8, 1800000, 1800000);
#endif
	...
	if (enable) 
	{
		rc= regulator_enable(vreg_8058_l8);
	} 
	else 
	{
		rc = regulator_disable(vreg_8058_l8);
	}
	if(rc)
	{
		pr_err("%s: regulator_enable %s = %d\n", __func__,	msm_wlan_regulator, rc);
...
/*+==============================================================+
| Function:      bcm_wlan_power_on								| 
   
| Description:   power control of wifi							|

| Input:   on_level = 1: power on, 2: reset on, 3: wake on		|                                                    
...
void bcm_wlan_power_on(int on_level)
{
	...
	if (!bcm43xx_init_flag) 
	{
		printk(KERN_ERR "%s: WLAN gpio don't init, bcm_wlan_power_on fail, bcm43xx_init_flag=%d \n",__func__, bcm43xx_init_flag);
		return;
	}

#if 1	
	/*select oob mod used for 4319*/	
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM43XX_WL_WAKE_HOST, 0, GPIO_CFG_INPUT,
									GPIO_CFG_NO_PULL, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
	if (rc) 
	{
		printk(KERN_ERR "%s:  Could not configure gpio:%d, rc=%d\n",__func__, GPIO_BCM43XX_WL_WAKE_HOST, rc);
		return;
	}
	gpio_set_value(GPIO_BCM43XX_WL_WAKE_HOST, 1);
#endif
	
	/* put HOST_WAKE_WL to HIGH */
	if (on_level == 1 || on_level == 3) {
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM43XX_HOST_WAKE_WL, 0, GPIO_CFG_OUTPUT, 
					GPIO_CFG_NO_PULL, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
		if (rc) 
			printk(KERN_ERR "%s: config HOST_WAKE_WL failed: %d\n",__func__, rc);
		gpio_set_value(GPIO_BCM43XX_HOST_WAKE_WL, 1);
		if (on_level == 3)
			return;
	}
	...
	if (on_level == 1) {
		...
		/*wlan reg_on set to high*/
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM43XX_REG_ON, 0, GPIO_CFG_OUTPUT, 
					GPIO_CFG_PULL_UP, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
		if (rc) 
			printk(KERN_ERR "%s: config WL_REG_ON high failed: %d\n",__func__, rc);
		gpio_set_value(GPIO_BCM43XX_REG_ON, 1);
		
		/* gpio 32 is for WL_WOW - WiFi reset */
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM43XX_WL_RST_N, 0, GPIO_CFG_OUTPUT, 
					GPIO_CFG_NO_PULL, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
		if (rc) {
			printk(KERN_ERR "%s: config WL_WOW failed: %d\n",__func__, rc);
		}
		gpio_set_value(GPIO_BCM43XX_WL_RST_N, 0);
		mmc_delay(100);
		gpio_set_value(GPIO_BCM43XX_WL_RST_N, 1);
...
/*+==============================================================+
| Function:      bcm_wlan_power_off								| 
   
| Description:   power control of wifi							|

| Input:   off_level = 1: power off, 2: reset off, 3: wake off	|  
...
void bcm_wlan_power_off(int off_level)
{
	...
	if (!bcm43xx_init_flag)
	{
		printk(KERN_ERR "%s: WLAN gpio don't init, bcm_wlan_power_off fail \n",__func__);
		return;
	}
	...
	if (off_level == 1) {
		...
		/*wlan reg_on set to low*/
		rc = gpio_tlmm_config(GPIO_CFG(GPIO_BCM43XX_REG_ON, 0, GPIO_CFG_OUTPUT, 
					GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
		if (rc) 
		{
			printk(KERN_ERR "%s: config WL_REG_ON low failed: %d\n",__func__, rc);
		}
		gpio_set_value(GPIO_BCM43XX_REG_ON, 0);
	...
	if (off_level == 1 || off_level == 3) {
		...
		gpio_set_value(GPIO_BCM43XX_HOST_WAKE_WL, 0);
...
static struct mmc_platform_data msm8x60_sdc4_data = {
	...
	.status			= msm8x60_wlan_status,
	.register_status_notify	= msm8x60_register_status_notify,
};
#endif
/*End: WLAN*/
...
#if 0
		rc = regulator_set_voltage(dsub_reg, 3000000, 3000000);
#else
...
static void display_common_power(int on)
{
	...
	if (machine_is_msm8x60_surf() || machine_is_msm8x60_ffa() ||
		...
		if (on) {
			...
			if (!display_reg)
				return;
			rc = regulator_set_voltage(display_reg,2850000, 2850000);
			if (rc)
				goto out;
...
static struct msm_bus_vectors mdp_1080p_vectors[] = { 
	...
	{ 
		...
		.dst = MSM_BUS_SLAVE_SMI, 
		.ab = 432384000, 
		.ib = 2640000000U, // SMI max freq at nominal, 330MHz * 8 
	}, 
	/* Master and slaves can be from different fabrics */ 
	{ 
		.src = MSM_BUS_MASTER_MDP_PORT0, 
		.dst = MSM_BUS_SLAVE_EBI_CH0, 
		.ab = 432384000, 
		.ib = 2048000000, // EBI max freq at nominal, 256 MHz * 8 
	}, 
}; 
#endif
...
#ifdef CONFIG_S7_FEATURE_OEMINFO
extern int __init rmt_oeminfo_add_device(void);
#endif
...
set_three_phase_freq_badass(CONFIG_CPU_FREQ_GOV_BADASS_3_PHASE_FREQ);
...
#if defined(CONFIG_BATTERY_MSM8X60) || defined(CONFIG_BATTERY_MSM8X60_S7PRO)	
	setup_charging_gpio_status(1, MAX8903_CEN_N, 0);
	setup_charging_gpio_status(1, USB_CHARGING_CTRL, 1);
#endif
	...
	if (machine_is_msm8x60_surf() || machine_is_msm8x60_ffa() || machine_is_msm8x60_fusn_ffa() ||
		machine_is_msm8x60_dragon()) {
...
#if defined(CONFIG_S7_FEATURE_HANDSET) || defined(CONFIG_S7_FEATURE_HANDSET_MODULE)
    platform_device_register(&hs_device);
#endif 
